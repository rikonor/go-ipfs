...
func (i *gatewayHandler) getOrHeadHandler(w http.ResponseWriter, r *http.Request) {
	...
	nd, err := core.Resolve(ctx, i.api.IpfsNode(), path.Path(urlPath))
	// If node is in offline mode the error code and message should be different
	// if err == core.ErrNoNamesys && !i.api.IpfsNode().OnlineMode() {
	...
	dr, err := uio.NewDagReader(ctx, nd, i.api.IpfsNode().DAG)
	...
	for _, link := range nd.Links {
		if link.Name == "index.html" {
			...
			nd, err := core.Resolve(ctx, i.api.IpfsNode(), path.Path(urlPath+"/index.html"))
			if err != nil {
				internalWebError(w, err)
				return
			}
